<template>
  <div>
    <div>
      <van-nav-bar :title="$t('m.Publish.task1')" left-text left-arrow @click-left="$router.go(-1)" />
    </div>
    <div>
...
<!--    <van-dropdown-menu >
	  ...
	  <div class="van-dropdown-menu van-hairline--top-bottom">
		  <div role="button" tabindex="0" class="van-dropdown-menu__item" @click="toshow1()">
			  <span class="van-dropdown-menu__title"><div class="van-ellipsis" >{{value_id1}}</div></span>
		  </div>
		 <div role="button" tabindex="0" class="van-dropdown-menu__item" @click="toshow2()">
		   <span class="van-dropdown-menu__title"><div class="van-ellipsis">{{value_id2}}</div>
		  </span>
		 </div>
	 </div>
    </div>
    ...
    <div class="zylist p20">
      <van-list v-model="loading" :finished="finished" :loading-text="$t('m.mission.mission21')" :finished-text="$t('m.Publish.task2')" @load="onLoad">
        <div class="f-flex zyitem box-shadow" v-for="(item,index) in tasklist" :key="index">
          ...
          <div>
            ...
            <p>
              <el-button type="primary" @click="draw(item.id)">{{$t('m.Publish.task8')}}</el-button>
            </p>
          </div>
        </div>
      </van-list>
    ...
    <van-dialog v-model="xinx" show-cancel-button @confirm="confirms">
      <div class="xinx">
        <p><van-icon name="question" /> {{$t('m.Publish.task9')}}</p>
      </div>
    </van-dialog>
  </div>
</template>
